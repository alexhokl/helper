...
func GetDatabaseDailector(pathToConnectionStringFile string) (gorm.Dialector, error) {
	if pathToConnectionStringFile == "" {
		return nil, fmt.Errorf("file path to database connection string is not set")
	}
	connectionString, err := iohelper.ReadFirstLineFromFile(pathToConnectionStringFile)
	if err != nil {
